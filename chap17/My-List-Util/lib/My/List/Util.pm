...
My::List::Util provides some list operations.
...
Return summation of values which looks like numeric.

=cut

sub sum {
    my $sum;
    $sum += $_ for grep { eval { defined && /\A[+-]?\d/ } } @_;
    $sum;
}
...
sub shuffle {
    my @deck = @_;
    return unless grep { defined $_ } @deck;
